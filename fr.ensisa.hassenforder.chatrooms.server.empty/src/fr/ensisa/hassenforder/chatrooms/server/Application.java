...
public class Application implements NetworkListener {
	...
	public OperationStatus connectCommandUser(String name, CommandSession session) {
		...
		if (user == null)
			return OperationStatus.UNKNOWN_USER;
		if (user.isConnected())
			return OperationStatus.ALLREADY_CONNECTED;
	...
	public OperationStatus disconnectUser(String name) {
		...
		if (user == null)
			return OperationStatus.UNKNOWN_USER;
		if (!user.isConnected())
			return OperationStatus.NOT_CONNECTED;
	...
	public OperationStatus createChannel(String name, String channel, ChannelType type) {
		...
		if (!user.isConnected())
			return OperationStatus.NOT_CONNECTED;
		if (model.getChannel(channel) != null)
			return OperationStatus.CHANNEL_EXISTS;
		Channel c = null;
		switch (type) {
		case FREE:
			c = new Channel(channel, type, null, user);
			break;
		case MODERATED:
			c = new Channel(channel, type, user, user);
			break;
		}
		if (c == null)
			return OperationStatus.CHANNEL_CREATION_FAILED;
		...
		if (type == ChannelType.MODERATED) {
			model.getPendingMessages()
					.add(new Message(c, count++, "system", "welcome moderator to channel " + c.getName()));
		}
	...
	public OperationStatus ChangeChannelSubscription(String name, String channel, boolean subscription) {
		...
		if (c == null)
			return OperationStatus.UNKNOWN_CHANNEL;
	...
	public OperationStatus SetApprobation(String name, int messageId, boolean approved) {
		...
		if (message == null)
			return OperationStatus.UNKNOWN_MESSAGE;
		if (approved)
